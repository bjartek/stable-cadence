...
access(all) contract NFTStorefrontV2 {
    ...
    access(all) struct SaleCut {
        ...
        access(all) let receiver: Capability<&{FungibleToken.Vault}>
        ...
        /// initializer
        ///
        init(receiver: Capability<&{FungibleToken.Vault}>, amount: UFix64) {
    ...
    access(all) resource Listing {
        ...
        access(contract) let marketplacesCapability: [Capability<&{FungibleToken.Vault}>]?
        ...
        access(all) fun purchase(
            ...
            let storeFrontPublicRef = self.owner!.getCapability<&NFTStorefrontV2.Storefront>(NFTStorefrontV2.StorefrontPublicPath)
        ...
        init (
            ...
            salePaymentVaultType: Type,
            saleCuts: [SaleCut],
            marketplacesCapability: [Capability<&{FungibleToken.Vault}>]?,
            storefrontID: UInt64,
            customID: String?,
            commissionAmount: UFix64,
            expiry: UInt64
    ...
    access(all) entitlement StorefrontManager
    ...
    access(all) resource Storefront  {
        ...
        ///
        access(all) view fun borrowListing(listingResourceID: UInt64): &Listing? {
